...
TEST_CASE("Factor Equality Operation") {
    ...
    auto AB_vars_tweaked = pgm::factor::rv_list{1,3};
...
TEST_CASE("Factor Index Sorting") {
    ...
    auto AB_permuted_vars = pgm::factor::rv_list{2,1};
...
TEST_CASE("Factor Product", "[factor][operation]")
{
    ...
    // aren't expected to yield strict equality.
      ...
      auto expected_vars = pgm::factor::rv_list{1,2,3};
      auto expected_data = xt::xarray<double>{
      0.25, 0.35, 0.08, 0.16,
      0.05, 0.07, 0.00, 0.00,
      0.15, 0.21, 0.09, 0.18};
      expected_data.reshape({3, 2, 2});
      auto expected_product = pgm::factor(expected_vars,
                                                 pgm::factor::data_array(expected_data));
    ...
    SECTION("Product of factor with itself") {
      ...
      auto expected_self_product = pgm::factor(expected_vars,
                                                      pgm::factor::data_array(expected_data));
